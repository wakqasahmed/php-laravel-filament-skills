#!/usr/bin/env python3
"""Offline contract checks for the Laravel testing conventions skill."""
import json
import re
...
SKILL = (EVAL_DIR.parent / "SKILL.md") if (EVAL_DIR.parent / "SKILL.md").exists() else (ROOT / "skills" / "laravel" / "laravel-testing" / "SKILL.md")
HELD_OUT = EVAL_DIR / "fixtures" / "held-out.json"
TUNING = EVAL_DIR / "fixtures" / "tuning.json"
CONTRACT_RULES = {
    "test database safety": r"Tests must run against disposable storage or a dedicated test database whose name \*\*clearly contains `test`\*\*",
    "sqlite in memory default": r"Sqlite in-memory \(`:memory:`\) is the safest default",
    "pest syntax preference": r"Prefer Pest's functional syntax",
    "expect assertions": r"Use `expect\(\)` assertions",
    "factories for test data": r"Use model factories \(`SomeModel::factory\(\)`\) to generate realistic per-test data",
...
VALID_DECISIONS = {"apply_convention", "diagnose_fix", "preserve_existing"}
...
if __name__ == "__main__":
    ...
    if failures:
        print("FAIL: deterministic laravel-testing contract checks")
        print("\n".join(f"- {failure}" for failure in failures))
        raise SystemExit(1)
    print("PASS: deterministic laravel-testing contract checks")
