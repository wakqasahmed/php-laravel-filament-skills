#!/usr/bin/env python3
"""Offline contract checks for the Laravel conventions skill."""
import json
import re
...
SKILL = (EVAL_DIR.parent / "SKILL.md") if (EVAL_DIR.parent / "SKILL.md").exists() else (ROOT / "skills" / "laravel" / "laravel-conventions" / "SKILL.md")
HELD_OUT = EVAL_DIR / "fixtures" / "held-out.json"
TUNING = EVAL_DIR / "fixtures" / "tuning.json"
CONTRACT_RULES = {
    "eloquent over raw sql": r"Eloquent over raw SQL",
    "form request validation": r"Form Request classes for all validation",
    "config over env": r"`config\('key'\)` instead of `env\('KEY'\)` outside config files",
    "queue async jobs": r"Queue jobs \(`ShouldQueue`\) for anything slow",
    "foreign key type match": r"Match the foreign key column's type exactly",
...
if __name__ == "__main__":
    ...
    if failures:
        print("FAIL: deterministic laravel-conventions contract checks")
        print("\n".join(f"- {failure}" for failure in failures))
        raise SystemExit(1)
    print("PASS: deterministic laravel-conventions contract checks")
