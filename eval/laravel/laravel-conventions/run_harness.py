#!/usr/bin/env python3
"""Run isolated enabled and disabled laravel-conventions trials."""
import argparse
import hashlib
import json
import os
...
def prepare_workspace(workspace: Path, agent: Path, case: dict, condition: str) -> None:
    ...
    if condition == "enabled":
        shutil.copy2(ROOT / "skills" / "laravel" / "laravel-conventions" / "SKILL.md", workspace / "SKILL.md")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--trials", type=int, choices=[3, 4, 5, 6], default=3)
