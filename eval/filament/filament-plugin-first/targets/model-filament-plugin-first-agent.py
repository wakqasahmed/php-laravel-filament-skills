#!/usr/bin/env python3
"""Model-invoking target agent for the filament-plugin-first outcome evaluation."""
...
def main() -> int:
    ...
    format_instruction = (
        "Return ONLY a valid JSON object with exact keys: 'decision', 'chosen_candidate', 'primary_reason'."
    )

    if skill_path and Path(skill_path).is_file():
        skill_text = Path(skill_path).read_text(encoding="utf-8")
        system_prompt = (
            "You are an expert engineer evaluating Filament plugin-first reuse vs. build decisions. "
        ...
        system_prompt = (
            "You are a helpful software engineering assistant evaluating Filament components. "
            f"{format_instruction}"
        )
