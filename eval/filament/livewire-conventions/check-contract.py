...
CONTRACT_RULES = {
    ...
    "wire model performance": r"wire:model\.live.*?default 150ms debounce",
    "deferred binding default": r"Deferred binding \(the default, unmodified `wire:model`\)",
...
if __name__ == "__main__":
    ...
    if failures:
        print("FAIL: deterministic livewire-conventions contract checks")
        print("\n".join(f"- {failure}" for failure in failures))
        raise SystemExit(1)
    print("PASS: deterministic livewire-conventions contract checks")
